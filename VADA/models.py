...
class Classifier(nn.Module):
    def __init__(self, large=False):
        ...
        self.feature_extractor = nn.Sequential(
            nn.InstanceNorm2d(n_features, momentum=1, eps=1e-3,),  # L-17
            nn.Conv2d(1, n_features, 3, 1, 1),  # L-16
...
class Discriminator(nn.Module):
    def __init__(self, large=False):
        ...
        self.disc = nn.Sequential(
            nn.Linear(n_features * 1 * 228, 100),#8/2
            nn.ReLU(True),
            nn.Linear(100, 1)
        )
    ...
    def forward(self, x):
        y = x.view(x.size(0), -1)
        return self.disc(y).view(y.size(0), -1)
        ...
        if self.decay > 0:
            for name, param in model.named_parameters():
                if name in self.params and param.requires_grad:
                    self.shadow[name] = self.shadow[name] - (1 - self.decay) * (self.shadow[name] - param.data)
