...
def cal_acc(loader, model, flag=True, fc=None):
    ...
    f1score= f1_score(all_label, torch.squeeze(predict).float(), average = 'weighted').astype(float)
    accuracy = torch.sum(torch.squeeze(predict).float() == all_label).item() / float(all_label.size()[0])
    return accuracy, predict, all_output, all_label, f1score
